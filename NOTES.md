...
**Run the full migration block in `supabase/schema.sql` against the live project** — it's cumulative and safe to re-run; covers the delete-food policy, the Storage bucket, `food_reviews.photo_url`, the `food_favorites` table/policies, and the new `wait_times` table/policies.
